...
class AboutMethods < Neo::Koan
  ...
  def test_calling_global_methods_with_wrong_number_of_arguments
    exception = assert_raise(ArgumentError) do # Raises ArgumentError
      my_global_method # Called with 0 arguments
    end
    assert_match(/wrong number of arguments \(given 0, expected 2\)/, exception.message)

    exception = assert_raise(ArgumentError) do # Raises ArgumentError
      my_global_method(1,2,3) # Called with 3 arguments
    end
  ...
  if before_ruby_version("2.7")   # https://github.com/edgecase/ruby_koans/issues/12
    def test_calling_private_methods_with_an_explicit_receiver
      ...
      assert_match /private method `my_private_method' called for #<AboutMethods:0x[0-9a-f]+>/, exception.message
    end
  end
